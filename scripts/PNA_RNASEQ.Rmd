...
load data:
```{r}
df <- read.delim("../data/PhoPQ_analysis_salcom/salcom_spi2genes_colgan.txt", header = T, sep="\t")
df_salcom <- data.frame(wildtype = df$WT.MEP, SPI2 = df$WT.InSPI2, 
                        PhoPQ_ko = df$X.Delta.phoP.Q.InSPI2, row.names = df$SL1344.Locus.ID)

tpm_salcom <- data.frame(sapply(df_salcom, function(x) as.integer(gsub(",","", x))), row.names = rownames(df_salcom))

tpm_salcom <- tpm_salcom[order(tpm_salcom$SPI2),]
str(tpm_salcom)
```
...
pvalues_spi2 <- data.frame(lapply(reslist, function(l) {
  logfc <- data.frame( sapply(names(l), function(r) {
    l[[r]]$table[all_spi2_genes,5]
  }), row.names = all_spi2_genes )
})) 
pvalues_spi2 <-ifelse(pvalues_spi2 < 0.01, x <- "*",x<-"") 
...
col_col_fun = colorRamp2(c(10,100,1000), c("lightgrey", "yellow", "red"))
ht_colgan <- Heatmap(tpm_salcom, cluster_rows = F, cluster_columns = F,
...
col_fun_cpm = colorRamp2(c(0, 50, 100), c("lightgrey", "yellow", "red"))
order = factor(c(rep("KFF", 3), rep("RXR", 3), rep("TAT", 3), "Control"))
ht_cpm <- Heatmap(spi2_cpm, cluster_rows = F, cluster_columns = F,
...
lgd = Legend(col_fun = col_fun, title = "mean log2FC", 
             at = c(-2, 0, 2), legend_width = unit(4, "cm"), legend_height = unit(5, "cm"),
             title_position = "leftcenter-rot")
lgd_col = Legend(col_fun = col_col_fun, title = "Transcripts per million",
             at = c(0, 100, 1000), legend_width = unit(4, "cm"), legend_height = unit(5, "cm"),
             title_position = "leftcenter-rot")


svg("../analysis/PhoPQ_Salcom/hm_salcom.svg", width = 10, height = 10)
draw(ht_list, ht_gap = unit(0.5, "cm"))
draw(lgd, x = unit(5, "cm"), y = unit(10, "cm"), just = c("left", "bottom"))
draw(lgd_col, x = unit(19, "cm"), y = unit(10, "cm"), just = c("left", "bottom"))
dev.off()
```
