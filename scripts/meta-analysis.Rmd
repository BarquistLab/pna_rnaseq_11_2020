...
GenewiseCounts <- read.csv(
  "../data/metaanalysis/ab_data.csv", row.names = 2, header = T, comment.char = "#")
...
tot <- merge(orthotable, PNATAT_de[,c(1,5)], by.y=0 ,by.x="SL1344_locustags.fna")
tot <- merge(tot, PNARXR_de[,c(1,5)], by.y=0 ,by.x="SL1344_locustags.fna")
tot <- merge(tot, PNAKFF_de[,c(1,5)], by.y=0 ,by.x="SL1344_locustags.fna")
...
pvs_de <- tot[(tot$MG1655_locustags.fna %in% de_ab),c(4,6,8,10,12)] #|(tot$SL1344_locustags.fna %in% degenes)
pvs_de <- pvs_de < 0.01
pvs_de <- ifelse(pvs_de, pvs_de <- "*",pvs_de<-"") 

logfc_de <- tot[(tot$MG1655_locustags.fna %in% de_ab),c(3,5,7,9,11)] # |(tot$SL1344_locustags.fna %in% degenes)
...
colnames(logfc_de) <- c("TAT-acpP", "RXR-acpP", "KFF-acpP", "Colistin", "Polymyxin")
...
ht2 <- Heatmap(logfc_de, name = "Log2 FC",
               col = col_fun,
               cluster_rows = F, cluster_columns = F, show_heatmap_legend = F,
               cell_fun = function(j, i, x, y, width, height, fill) {
                 grid.text(sprintf("%.1s", pvs_de[i, j]), x, y, gp = gpar(fontsize = 10))
               },column_split = as.factor(c(rep("",3), rep(" ", 2))), column_gap = unit(0.3, "cm"),
               border = F, width = unit(5, "cm"), rect_gp = gpar(col = "darkgrey", lwd = 0.5),
               row_names_gp = gpar(fontsize =9), column_names_gp = gpar(fontsize =10),
              # row_names_max_width = max_text_width(rownames(logfc_de),gp = gpar(fontsize = 0)
              # ), column_names_max_height=max_text_width(colnames(logfc_de),gp = gpar(fontsize = 40)),
               #width = unit(20, "cm"), height = unit(45, "cm"), row_dend_width = unit(6, "cm"),
               #column_dend_height = unit(3, "cm"), 
               column_names_rot = 45)

svg("../analysis/orurke_analysis/antibiotics_hm_test.svg")
ht2
lgd2 = Legend(col_fun = col_fun, title = expression("Log"[2]*" FC"),grid_width = unit(0.5, "cm"), 
              legend_height = unit(2, "cm"), title_gp = gpar(fontsize =10), title_position = "leftcenter-rot", at = c(-2,-1,0,1,2),
              labels_gp = gpar(fontsize = 7 ), labels = c("-2", "-1", " 0"," 1", " 2" ))
draw(lgd2, x = unit(5, "cm"), y = unit(10, "cm"), just = c("right", "top"))
dev.off()
